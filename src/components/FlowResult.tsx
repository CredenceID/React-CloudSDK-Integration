...
} from "@/api/types/iso18013Service";
...
// Renders the Identity + Authentication pair for a single credential.
// Reused once for a single-document result, and once per entry when the
// wallet presented more than one document in the same session.
function DocumentPanel({ identity, auth, fallbackSuccess, label }: DocumentPanelProps) {
  const authStatus = auth.authenticationResult ?? (fallbackSuccess ? "SUCCESS" : "FAILED");
  const isSuccess = authStatus === "SUCCESS" || fallbackSuccess;
  ...
  return (
    <div className="flow-result-doc">
      {label && <div className="flow-result-doc-label">{label}</div>}
...
export function FlowResult({ data, onClose }: FlowResultProps) {
  ...
  // Multi-document profile: render one panel per credential the wallet
  // presented. Falls back to the single top-level identity/authentication
  // pair when the backend only returned one document.
  const documents = parsed?.documents?.length
    ? parsed.documents
    : [{ docType: undefined, identity, authentication: auth }];
      ...
      <div className="flow-result-body">
        {documents.map((doc, i) => (
          <DocumentPanel
            ...
            fallbackSuccess={data.res}
            label={
              documents.length > 1
                ? (doc.docType ?? `Document ${i + 1}`)
                : undefined
            }
          />
        ))}
